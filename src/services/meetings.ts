...
export const AUDIO_UPLOAD_POLL_TIMEOUT_MS = 15 * 60 * 1000;
export const AUDIO_UPLOAD_PENDING_MESSAGE = '音频上传耗时较长，后台仍在继续处理，请稍后刷新音频列表确认结果';

type AudioUploadPendingError = Error & {
  code: 'audio_upload_pending';
  taskId: string;
};
...
export const createAudioUploadPendingError = (taskId: string): AudioUploadPendingError => {
  const error = new Error(AUDIO_UPLOAD_PENDING_MESSAGE) as AudioUploadPendingError;
  error.name = 'AudioUploadPendingError';
  error.code = 'audio_upload_pending';
  error.taskId = taskId;
  return error;
};

export const isAudioUploadPendingError = (error: any): error is AudioUploadPendingError =>
  error?.code === 'audio_upload_pending';
